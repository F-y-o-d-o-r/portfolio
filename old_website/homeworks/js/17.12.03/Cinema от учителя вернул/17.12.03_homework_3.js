...
var cinema = (function () {
  $(document).ready(function () {
    ...
    $('body').on('change', '[name="select_little_big"]', function () {
      ...
      */ //!!!ПРЕДСТАВЬТЕ, ЧТО У ВАС 10,20 option КАЖДЫЙ СО СВОИМ value - ПРИДЕТСЯ СТАВИТЬ 20 if - ООП ЗАТЕМ И НУЖНО, ЧТОБЫ ОБОЙТИ ТАКИЕ СИТУАЦИИ...
       cinema.x=$('[name="select_little_big"] :selected').val();  //!!!+ ВОТ ТУТ cinema., А НЕ this - ДУМАЮ, ПОНИМАЕТЕ, ПОЧЕМУ...
    });
  });
  ...
  return {
    ...
    func_select: function (seats_count) {
      ...
      this.func_free();                        //!this
      this.func_seats_select(seats_count);     //!this
    },
    func_free: function () {
      arr_free = new Array();
      count_free = 0;
    ...
    func_seats_select: function (seats_count) {
      if (seats_count <= arr_free.length) {
        ...
        //cinema.count_free = Math.floor(Math.random() * arr_free.length);  //!ТУТ Я НЕ УСПЕЛ ВАМ СКАЗАТЬ - ЕСЛИ ВЫ НЕ СОЗДАЕТЕ В ДРУГОМ МЕТОДЕ count_free (КАК И arr_free) ЧЕРЕЗ var, ТО МОЖНО "ПРЯМО"...
