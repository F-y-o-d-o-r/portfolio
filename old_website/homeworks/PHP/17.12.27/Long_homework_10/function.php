...
if (isset($_POST['search_what']) and !empty($_POST['search_what'])) {
  ...
  if (!isset($_POST['show'])) {
    ...
    //echo 'var interv = setInterval(\'func_img()\', 2000);';
    echo '</script>';
    foreach ($massData as $massData_index => $massData_value) {
      ...
      //echo "var interv = setInterval('func_img()', 1500);";
      echo '</script>';
    }
...
...в тот, что я высылал mb_strtoupper($masf) - пропустил + если бы использовали там еще не до конца - нужно решить проблему задвоения рисунков... но это тут не суть... ваш вопрос сейчас посмотрю...
*/