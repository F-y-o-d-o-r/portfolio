...
if (isset($_POST['name_select']) or isset($_POST['poisk']) or isset($_POST['tovar']) or isset($_POST['tov_insert']) or isset($_POST['tov_update']) or isset($_POST['tov_delete']) or isset($_POST['klient']) or isset($_POST['klient_insert']) or isset($_POST['klient_update']) or isset($_POST['klient_delete'])) {                     // 13. ЕСЛИ НАЖАТА КНОПКА "ПОИСК"...
  switch (key($_POST)) {
    case name_select:
...
arr_tr[i].childNodes[0].style.visibility="hidden";
    ...
    case klient:
      ...
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'tbody_klient', '<td>', '</td>', '<tr>', '</tr>', '', '');
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'tbody_klient', '<td>', '</td>', "<tr style=\'cursor:pointer;\' onclick=\'klient_update_delete(this);\' onmouseover=\'kursor_start(this);\' onmouseout=\'kursor_end(this);\'>", '</tr>', '', '');
      echo '<script>
...
arr_tr[i].childNodes[0].style.visibility="hidden";
    ...
    case klient_insert:
      ...
      mysqli_query($obj->link_db, $query);
      $query = "select id_user, name, tel, adr from user ORDER BY name";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, '', '<td>', '</td>', '<tr style="cursor:pointer;" onclick="klient_update_delete(this);" onmouseover="kursor_start(this);" onmouseout="kursor_end(this);">', '</tr>', '', '');
      echo '<script>
var arr_tr=tbody_klient.getElementsByTagName("tr");
for(var i=0; i<arr_tr.length; i++) {
arr_tr[i].childNodes[0].style.visibility="hidden";
      ...
      break;
    case klient_update:
      $query = "update user set name = '" . $_POST['klient_name_pole'] . "' where id_user = " . $_POST['id_pole'];//todo add
      mysqli_query($obj->link_db, $query);
      $query = "select id_user, name, tel, adr from user ORDER BY name";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, '', '<td>', '</td>', '<tr style="cursor:pointer;" onclick="klient_update_delete(this);" onmouseover="kursor_start(this);" onmouseout="kursor_end(this);">', '</tr>', '', '');
      echo '<script>
...
arr_tr[i].childNodes[0].style.visibility="hidden";
      ...
      break;
    case klient_delete:
      $query = "delete from user where id_user = " . $_POST['id_pole'];//todo add
      mysqli_query($obj->link_db, $query);
      $query = "select id_user, name, tel, adr from user ORDER BY name";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, '', '<td>', '</td>', '<tr style="cursor:pointer;" onclick="klient_update_delete(this);" onmouseover="kursor_start(this);" onmouseout="kursor_end(this);">', '</tr>', '', '');
      echo '<script>
