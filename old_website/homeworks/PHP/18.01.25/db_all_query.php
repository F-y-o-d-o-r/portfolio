...
dat date not null
   ...
   join tovar on pokup.id_tovar=tovar.id_tovar";       // ...ФОРМИРУЕТСЯ СТРОКА ЗАПРОСА ДЛЯ "0-ГО" ВЫВОДА-ТАБЛИЦА...
    ...
    case tovar:
      ...
      $obj->File_system('tovar', '/domains/homeworks/2018.01.25/img/tmp/', '');
      /*$arr_dir = scandir($dir);
...
arr_tr[i].childNodes[0].style.visibility="hidden";
    ...
    case tov_insert:
      ...
      //$dir = '/domains/homeworks/2018.01.25/';
      $tov_img = mysqli_real_escape_string($obj->link_db, $_POST['tov_img']);
      $obj->File_system('insert', '/domains/homeworks/2018.01.25/', $tov_img);
      ...
      $query = "insert into tovar(tov, image) values('" . $tov . "','" . DB_class::$imgg . "')";
      mysqli_query($obj->link_db, $query);
      $query = "select id_tovar,tov, image from tovar order by tov";
...
arr_tr[i].childNodes[0].style.visibility="hidden";
    ...
    case tov_update:
      ...
      //$dir = '/domains/homeworks/2018.01.25/';
      $tov_img = mysqli_real_escape_string($obj->link_db, $_POST['tov_img']);
      $obj->File_system('update', '/domains/homeworks/2018.01.25/', $tov_img);
      /*if (strpos($tov_img, 'tmp')) {
        $imgg = substr($tov_img, 11);
        $imgg = 'img/a' . $imgg;
        rename("$dir/$tov_img", "$dir/$imgg");
      ...
      $tov = mysqli_real_escape_string($obj->link_db, $_POST['tov_pole']);
      $query = "update tovar set tov='" . $tov . "',image = '" . DB_class::$imgg . "' where id_tovar=" . $id;
      mysqli_query($obj->link_db, $query);
      $query = "select id_tovar,tov, image from tovar order by tov";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, '', '<td onmouseover="kursor_start(this);" onmouseout="kursor_end(this);">', '</td>', '<tr style="cursor:pointer;" onclick="tov_update_delete(this);">', '</tr>', '', '');
      echo '<script>
...
arr_tr[i].childNodes[0].style.visibility="hidden";
    ...
    case tov_delete:
      ...
      //$dir = '/domains/homeworks/2018.01.25/';
      $tov_img = mysqli_real_escape_string($obj->link_db, $_POST['tov_img']);
      $obj->File_system('delete', '/domains/homeworks/2018.01.25/', $tov_img);
...
jquery_send("#okno", "post", "db_poisk.php", ["poisk"], []);
    ...
    case tov_pokup_select:
      ...
      break;
    case pokup:
      $query = "select concat(id_user, '#', name) as users from user order by users";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'sel_name_id_pokup', '<option>', '</option>', '', '', '<option>Все---</option>', '');
      echo '<script>
func_select_extract("form_pokup", "sel_name_pokup");
</script>';
      $query = "select concat(id_tovar, '#', tov) as tovars from tovar order by tovars";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'sel_tov_id_pokup', '<option>', '</option>', '', '', '<option>Все---</option>', '');
      echo '<script>
func_select_extract("form_pokup", "sel_tov_pokup");
</script>';
      $query = "select concat(id_day, '#', day) as days from day order by days";
      $day = getdate()[mday];
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'day_pokup_id', '<option>', '</option>', '', '', '<option> ' . $day . '</option>', '');
      echo '<script>
func_select_extract("form_pokup", "day_pokup");
</script>';
      $query = "select concat(id_month, '#', month) as months from month order by months";
      $mon = getdate()[mon];
      if (strlen($mon) == 1) {
        $mon = '0' . $mon;
      }
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'month_pokup_id', '<option>', '</option>', '', '', '<option>' . $mon . '</option>', '');
      echo '<script>
func_select_extract("form_pokup", "month_pokup");
</script>';
      $query = "select concat(id_yar, '#', yar) as years from yar order by years";
      $yar = getdate()[year];
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'yar_pokup_id', '<option>', '</option>', '', '', '<option>' . $yar . '</option>', '');
      echo '<script>
func_select_extract("form_pokup", "yar_pokup");
</script>';
      break;
    case pokup_insert:
      echo print_r($_POST);
      $use = $_POST['name_id'];
      $pok = $_POST['tov_id'];
      $dat1 = $_POST['dat_pokup_ins'];
      //$dat1 = explode('-', $_POST['dat_pokup_ins']);
      //$yar = $dat1[0];
      //$mon = $dat1[1];
      //$day = $dat1[2];
      $query = "INSERT INTO pokup (id_user, id_tovar, dat) VALUES ('$use', '$pok', '$dat1')";
      mysqli_query($obj->link_db, $query);
      echo '<script>
jquery_send(\'#okno\', \'post\', \'db_poisk.php\', [\'poisk\'], []);
</script>';
