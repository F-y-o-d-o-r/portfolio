...
//ДЕЛАЕМ ДВУМЕРНЫЙ МАССИВ МАРКА-МОДЕЛЬ-ЦЕНА
    ...
    //ДЕЛАЕМ МАССИВ ЗАГРУЖЕННЫХ ФОТО
    self::$fromImg = explode('%', $_SESSION['img'], -1);
    //пройти по массиву
    /*foreach ($mas12 as $key => $value) {
      foreach ($value as $key2 => $value2) {
        //!!!ТАКОЕ СООТНОШЕНИЕ НЕ ВЕРНО - ЭТО ДВА АБСОЛЮТНО РАЗНЫЕ ПО СТРУКТУРЕ МАССИВЫ (ДВУМЕРНЫЙ ДАННЫХ И ОДНОМЕРНЫЙ img
        //Т.Е. - НУЖНО БРАТЬ ЗНАЧЕНИЕ ИЗ ПЕРВОГО МАССИВА-ДАННЫХ (НАПРИМЕР, ACER) И ИСКАТЬ (ДОПОЛНИТЕЛЬНЫМ ПЕРЕБОРОМ, ЧТО НЕКРАСИВО
        //ИЛИ in_array, ЧТО У МЕНЯ) СООТВЕТСТВИЕ ВО ВТОРОМ МАССИВЕ-img)
        //if (mb_strtolower($value2) == strstr(self::$fromImg[$key2], ".", true))  {
        if(in_array(mb_strtolower($value2.'.jpg'), self::$fromImg)) {
          $img_add=self::$fromImg[array_search(mb_strtolower($value2.'.jpg'), self::$fromImg)];
          //$mas12[$key][] = '<img src="' . self::$fromImg[$key2] . '">';
          $mas12[$key][] = '<img src="' . $img_add . '">';
          $mas12[$key][] = date('d-m-Y');
          //print_r($mas12[$key]);
        }
      }
    }*/
    foreach ($mas12 as $key => $value) {
      if (in_array(mb_strtolower($value[0] . '.jpg'), self::$fromImg)) {
        $img_add = self::$fromImg[array_search(mb_strtolower($value[0] . '.jpg'), self::$fromImg)];
        $mas12[$key][] = '<img src="' . $img_add . '">';
        $mas12[$key][] = date('d-m-Y');
      }
    }
