  ...
  {                                               // 17. МЕТОД БАЗОВОГО КЛАССА "ВЫВОД В DOM"
    ...
    foreach ($this->row_db as $k1 => $mas1) {                                //ПЕРЕБОР $this->row_db - ФОРМИРОВАНИЕ СТРОК
      ...
      foreach ($mas1 as $k2 => $mas2) {
        ...
        if ($k2 == 'up') {
          $mas2 = "<a href='#' id='$mas2' style='color:blue;' onclick='jquery_send(" . $jq_se . "); return false;'>" . $mas2 . "</a>";
...
//***********************************************************
  function File_system($case, $dir, $file)
  {
    switch ($case) {
      case tovar:
        $arr_dir = scandir($dir);
      ...
      case insert:
        if (strpos($file, 'tmp')) {
          self::$imgg = substr($file, 11);
          self::$imgg = 'img/a' . self::$imgg;
          rename("$dir/$file", "$dir/" . self::$imgg);
        } else {
          self::$imgg = 'img/nophoto.jpg';
        }
        break;
      case update:
        if (strpos($file, 'tmp')) {
          ...
          self::$imgg = 'img/a' . self::$imgg;
          rename("$dir/$file", "$dir/" . self::$imgg);
          $this->query = "select image from tovar where id_tovar =" . intval($_POST['id_pole']);
          $this->DB_query_all();
          $img_old = $this->row_db[0]['image'];
...
///////////////////////////////////////////////////////////////////
class DB_all extends DB_class
{                                       // 19. ПРОИЗВОДНЫЙ КЛАСС
  function Query_Dom_all($link, $query, $id_null, $tag1, $tag11, $tag2, $tag22, $tag3, $tag33)
