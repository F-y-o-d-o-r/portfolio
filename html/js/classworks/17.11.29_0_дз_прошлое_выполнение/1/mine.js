...
function func_validate(text_value) {
  var r = /^[а-яэіїє\w]+$/i;
  if (r.test(text_value)) {
    title_error.style.visibility = "hidden";
    var rBody = "(" + text_value + ")";
    var rKey = "gi";
    var r_text = new RegExp(rBody, rKey);
    //tbod.innerHTML=tbod.innerHTML.replace(r_text, "<span style='background-color:gold;'>$1</span>");
    var arr_tr = document.getElementById("tbod").getElementsByTagName("tr");
    for (var i = 0; i < arr_tr.length; ++i) {
      for (var j = 0; j < arr_tr[i].childNodes.length; ++j) {
        if (j != 0) {
          arr_tr[i].childNodes[j].innerHTML = arr_tr[i].childNodes[j].innerHTML.replace(r_text, "<span style='background-color:gold;'>$1</span>");
        }
      }
    }
  }
