...
foreach($this->row_db as $k1=>$mas1) {                                //ПЕРЕБОР $this->row_db - ФОРМИРОВАНИЕ СТРОК
   ...
   foreach($mas1 as $k2=>$mas2) {
     ...
     if($k2=='up') {$mas2="<a href='#' id='$mas2' style='color:blue;' onclick='jquery_send(".$jq_se."); return false;'>".$mas2."</a>";} //ПРИ КАЖДОМ "НУЛЕВОМ" - В ЯЧЕЙКУ ТАБЛИЦЫ <a> С id=id_user/id_pokup (ДЛЯ select-МЕНЮ(ТОВАРЫ) - ПРОСТО НЕ БУДЕТ РАБОТАТЬ, Т.К. НЕТ $k2=='up')
      if($k2=='image') {$mas2="<img src='".$mas2."' alt='' />";}
