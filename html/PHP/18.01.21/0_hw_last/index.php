...
<style>
 td {background-color:white; border:1px solid silver; color:teal; font-size:12pt;}
</style>
</head>
<body>
...
function color_font(elem) {                 // 3.js-ФУНКЦИЯ "ВЫДЕЛЕНИЯ" ВЫБРАННОГО ПУНКТА МЕНЮ (li)
 ...
 for(var i=0; i<spis.length; i++) {
   if(spis[i]==elem) {spis[i].setAttribute("style", "color:royalblue; font-weight:bold;");}
   else {spis[i].setAttribute("style", "color:teal; font-weight:normal;");}
 }
}
</script>
