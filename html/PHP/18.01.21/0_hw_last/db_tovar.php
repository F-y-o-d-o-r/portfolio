<br>
<div id="tovar">  <!-- !!! 2. .«html-КАРКАС» - ФОРМА (ПОЛЕ + 3-РИ КНОПКИ (ДОБАВЛЕНИЕ(insert), КОРРЕКТИРОВКА(update), УДАЛЕНИЕ(delete)) -->
<br>
<div id="tovar_korr" style="background-color:gainsboro; margin-left:6%; margin-right:6%; border:1px solid silver; border-radius:4px; text-align:justify; padding:3px;">
...
<form style="margin:0px;">              <!-- !!! ПОЛЕ ТИПА  hidden (НЕОБХОДИМОСТИ В ЕГО ОТОБРАЖЕНИИ НЕТ) БУДЕТ НЕОБХОДИМО ДЛЯ СОХРАНЕНИЯ id_tovar -->
<input type="hidden" name="tov_id">     <!-- !!! 3. На КАЖДУЮ КНОПКУ формы «ВЕШАЕТСЯ» ПРЕДВАРИТЕЛЬНАЯ (ПЕРЕД ajax) js-onclick-ОБРАБОТКА (ПРОВЕРКА ВВЕДЕННОЙ ИНФОРМАЦИИИ и ИЗМЕНЕНИЕ СТИЛЕЙ ЭЛЕМЕНТОВ ФОРМЫ (АКТИВАЦИЯ/ДЕАКТИВАЦИЯ, ФОН)) -->
<input type="text" style="background-color:silver; width:100%;" name="tov_pole" onkeypress="if(event.which==13) return false;">
...
<tbody>
<tr>
<td style="width:5%; visibility:hidden;"></td>   <!-- !!! 4. ТАБЛИЦА будет иметь ДВА ПОЛЯ (id_tovar (БУДЕТ «НЕВИДИМЫМ» (visibility:hidden;), НЕОБХОДИМО ДЛЯ update-delete where id_tovar) и tov (НЕПОСРЕДСТВЕННО – ОТОБРАЖЕНИЕ ТОВАРА)) -->
