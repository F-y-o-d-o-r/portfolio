...
if (isset($_POST['name_select']) or isset($_POST['poisk']) or isset($_POST['tovar']) or isset($_POST['tov_insert']) or isset($_POST['tov_update']) or isset($_POST['tov_delete']) or isset($_POST['klient']) or isset($_POST['klient_insert']) or isset($_POST['klient_delete']) or isset($_POST['klient_update']) or isset($_POST['sort_top']) or isset($_POST['sort_bottom']) or isset($_POST['pokup_update_delete']) or isset($_POST['pokup_delete']) or isset($_POST['pokup_update']) or isset($_POST['name_pokup_select']) or isset($_POST['tov_pokup_select']) or isset($_POST['pokup']) or isset($_POST['pokup_insert'])) {                     // 13. ЕСЛИ НАЖАТА КНОПКА "ПОИСК"...
  switch (key($_POST)) {
    case name_select:
...
//
      /*
      $dirr='/home/forum/www/img/tmp/';  //ФИЗИЧЕСКИ ЧИСТИМ ОТ «СТАРОГО»
      $masdir=scandir($dirr);
      foreach($masdir as $masdirr) {
        unlink("$dirr/$masdirr");
      }
      */
      $obj->File_system('tovar', '/home/forum/www/img/tmp/', '');
//
...
arr_tr[i].childNodes[0].style.visibility="hidden";
    ...
    case tov_insert:
      ...
      */
      $file_img_name = $obj->File_system('insert', '/home/forum/www/', $tov_img);
      $tov = mysqli_real_escape_string($obj->link_db, $_POST['tov_pole']);
      $query = "insert into tovar(tov, image) values('" . $tov . "', '" . $file_img_name . "')";
      mysqli_query($obj->link_db, $query);
      $query = "select id_tovar,tov, image from tovar order by tov";
...
arr_tr[i].childNodes[0].style.visibility="hidden";
    ...
    case tov_update:
      ...
      */
      $file_img_name = $obj->File_system('update', '/home/forum/www/', $tov_img);
//$tov=mysqli_real_escape_string($obj->link_db, $_POST['tov_pole']);
//$id=intval($_POST['id_pole']);
      $query = "update tovar set tov='" . $tov . "', image='" . $file_img_name . "' where id_tovar=" . $id;
      mysqli_query($obj->link_db, $query);
      $query = "select id_tovar,tov, image from tovar order by tov";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, '', '<td onmouseover="kursor_start(this);" onmouseout="kursor_end(this);">', '</td>', '<tr style="cursor:pointer;" onclick="tov_update_delete(this);">', '</tr>', '', '');
      echo '<script>
...
arr_tr[i].childNodes[0].style.visibility="hidden";
      ...
      break;
    case tov_delete:
      $tov_img = mysqli_real_escape_string($obj->link_db, $_POST['tov_img']);  // «ПРИХОДИТ» img/a….jpg, А НЕОБХОДИМ…
      /*
      $dir='/home/forum/www/';  // ДЛЯ Unix-УДАЛЕНИЯ
      if(!strpos($tov_img, 'nophoto.jpg')) {
        unlink("$dir/$tov_img");  //«ФИЗИЧЕСКОЕ» УДАЛЕНИЕ С ДИСКА (В ЛЮБОМ МЕСТЕ)
      }
      */
      $obj->File_system('delete', '/home/forum/www/', $tov_img);
//
      $id = intval($_POST['id_pole']);
...
jquery_send("#okno", "post", "db_poisk.php", ["poisk"], []);
    ...
    case name_pokup_select:
      ...
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'tel_pole_pokup_id', '', '', '', '', '', '');  //В НУЖНОЕ ПОЛЕ…
      $query = "select adr from user where name='" . $name_pokup_sel . "'";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'adr_pole_pokup_id', '', '', '', '', '', '');  //В НУЖНОЕ ПОЛЕ…
      $query = "select id_user from user where name='" . $name_pokup_sel . "'";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'name_id', '', '', '', '', '', '');  //В НУЖНОЕ ПОЛЕ (В «СКРЫТЫЙ» span ПОДСТАВЛЯЕТСЯ id КЛИЕНТА, НУЖЕН ДЛЯ ДАЛЬНЕЙШЕГО insert (update, delete – ЗДЕСЬ МОЖЕТ НЕ ИСПОЛЬЗОВАТЬСЯ)…
      break;
    case tov_pokup_select:
      ...
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'tov_id', '', '', '', '', '', '');  //В НУЖНОЕ ПОЛЕ (В «СКРЫТЫЙ» span ПОДСТАВЛЯЕТСЯ id ТОВАРА, НУЖЕН ДЛЯ ДАЛЬНЕЙШЕГО insert (update, delete – ЗДЕСЬ МОЖЕТ НЕ ИСПОЛЬЗОВАТЬСЯ)…
      $query = "select image from tovar where tov='" . $tov_pokup_sel . "'";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'pokup_img', '', '', '', '', '', '');  //В НУЖНОЕ ПОЛЕ (ПРОСТО В div, НО В db_class.php «АВТОМАТОМ ЗАВОРАЧИВАЕТСЯ» В <img, ЕСЛИ ПОЛЕ 'image')…
      break;
    case pokup_update:
...
jquery_send("#okno", "post", "db_poisk.php", ["poisk", "marker"], ["", "' . $name_id . '/' . $pok_id . '"]);
    ...
    case pokup:
      $query = "select name from user order by name";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'sel_name_id_pokup', '<option>', '</option>', '', '', '<option>Выберите---</option>', '');
      $query = "select tov from tovar order by tov";
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'sel_tov_id_pokup', '<option>', '</option>', '', '', '<option>Выберите---</option>', '');
      $query = "select day from day order by day";  //НУЖНА НОВАЯ ТАБЛИЦА
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'day_pokup_id', '<option>', '</option>', '', '', '', '');
      $query = "select month from month order by month";   //НУЖНА НОВАЯ ТАБЛИЦА
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'month_pokup_id', '<option>', '</option>', '', '', '', '');
      $query = "select yar from yar order by yar";   //НУЖНА НОВАЯ ТАБЛИЦА
      $obj_query_tag->Query_Dom_all($obj->link_db, $query, 'yar_pokup_id', '<option>', '</option>', '', '', '', '');
      break;
    case pokup_insert:
      $name_id_pokup = intval($_POST['name_id']); //id КЛИЕНТА
      $tov_id_pokup = intval($_POST['tov_id']);  //id ТОВАРА
      $dat_pokup = mysqli_real_escape_string($obj->link_db, $_POST['dat_pokup_ins']); //ДАТА ПОКУПКИ
      $query = "insert into pokup(id_user, id_tovar, dat) values(" . $name_id_pokup . "," . $tov_id_pokup . ",'" . $dat_pokup . "')";
      mysqli_query($obj->link_db, $query);
//
      $obj->query = "select max(id_pokup) as max_id_pokup from pokup";
      $obj->DB_query_all();
//
      echo '<script>
jquery_send("#okno", "post", "db_poisk.php", ["poisk", "marker"], ["", "' . $name_id_pokup . '/' . $obj->row_db[0]['max_id_pokup'] . '"]);
</script>';
      break;
  }
