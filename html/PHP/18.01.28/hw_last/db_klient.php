...
<div id="klient">
  ...
  <div id="klient_korr"
    ...
    <a href="#" style="color:blue; font-weight:bold; font-size:11pt;"
       onclick="klient_insert();return false;">Добавить</a> нового клиента или <span
        style="color:teal; font-weight:bold; font-size:11pt;">выберите из таблицы (изменение, удаление)</span>
    <form style="margin:0px;">
      ...
      <div style
      "display:table-raw;">
      <div style="display:table-cell; width:70px; float:left;">Клиент</div>
      <div style="display:table-cell; width:360px;"><input type="text" maxlength="50"
                                                           style="background-color:silver; width:360px;"
                                                           name="klient_name_pole"
                                                           onkeypress="if(event.which==13) return false;"></div>
  </div>
  <div style
  "display:table-raw;">
  <div style="display:table-cell; width:70px; float:left;">Телефон</div>
  <div style="display:table-cell; width:360px;"><input type="text" maxlength="25"
                                                       style="background-color:silver; width:360px;"
                                                       name="klient_tel_pole"
                                                       onkeypress="if(event.which==13) return false;"></div>
</div>
<div style"display:table-raw;">
<div style="display:table-cell; width:70px; float:left;">Адрес</div>
<div style="display:table-cell; width:360px;"><input type="text" maxlength="50"
                                                     style="background-color:silver; width:360px;"
                                                     name="klient_adr_pole"
                                                     onkeypress="if(event.which==13) return false;"></div>
</div>
</div>
<br>
<input type="button" style="background-color:silver;" name="klient_insert" value="Добавить"
       onclick="klient_insert_prov(this.name, klient_name_pole.value, klient_tel_pole.value, klient_adr_pole.value);">
<input type="button" style="background-color:silver;" name="klient_update" value="Изменить"
       onclick="klient_update_knop();">
<input type="button" style="background-color:silver;" name="klient_delete" value="Удалить"
       onclick="klient_delete_prov(this.name, klient_id.value);">
...
<script>
  ...
  function klient_insert_prov(name, pole_name, pole_tel, pole_adr) {
    ...
    for (var i = 0; i < mas_a.length; i++) {
      if (mas_a[i].innerHTML.toLowerCase().indexOf(pole_name.toLowerCase()) != -1) {
        priz = 1;
      }
    }
